...
class OrderAnalyticsController extends Controller
{
    public function restaurantTrends(Request $request, $restaurantId)
    {
        ...
        $dailyData = DB::select("
            ...
            ) as dates on date(orders.order_time) = dates.date
            where orders.restaurant_id = ?
            and orders.order_time between ? and ?
            group by date(order_time)
            group by date
        ", [$restaurantId, $restaurantId, $startDate, $endDate, $restaurantId, $startDate, $endDate]);
        
    
        $summary = DB::select("
            select count(*) as totalOrders, sum(order_amount) as totalRevenue, avg(order_amount) as avgOrderValue from orders where restaurant_id = ?
            and order_time between ? and ?
    ...
    public function topRestaurants(Request $request)
    {
        ...
        $topRestaurants = DB::select("
            select r.id as restaurantId, r.name as restaurantName, r.address as restaurantAddress,count(o.id) as totalOrders,sum(o.order_amount) as totalRevenue, avg(o.order_amount) as avgOrderValue
            from restaurants r
    ...
    {
        $query = "select o.*, r.name as restaurantName, r.address as restaurantAddress
            FROM orders o
        ...
        $countQuery = "select count(*) as total from (" . str_replace('o.*, r.name as restaurantName, r.address as restaurantAddress', 'o.id', $query) . ") as countTable";
